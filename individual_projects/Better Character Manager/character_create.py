...
def character_generate(character_index):
    #stat generation
    def stat_gen():
    ...
    if randomrace == 1:
        ...
        randomrace = 'elf'
    print(f"Here are the available races: {', '.join(races)}")
    raceinput = randomrace

    #get character class
    randomclass = random.randint(1, 3)
    if randomclass == 1:
        randomclass = 'cleric'
    elif randomclass == 2:
        randomclass = 'fighter'
    ...
    #get initial inventory
    print(f"\nSetting {name.capitalize()}'s inventory...")
    character_index[name]['inventory'] = [fake.word(), fake.word(), fake.word(), fake.word(), fake.word(), fake.word(), fake.word(), fake.word()]
    character_index[name]['stats'] = {}
    ...
    match uinput:
        ...
        case '2': #get stat inputs (max 20)
            stat_list = []
            for i in range(4):
                stat_list.append(random.randint(1, 20))
